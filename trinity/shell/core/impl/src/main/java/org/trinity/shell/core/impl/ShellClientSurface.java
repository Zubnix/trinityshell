...
// TODO documentation
/**
 * A <code>ShellClientSurface</code> wraps a {@link DisplayRenderArea} that was created
 * by an independent program. As such the visual content can not be directly
 * manipulated. A <code>ShellClientSurface</code> is the owner of the
 * <code>PlatformRenderArea</code> it wraps. A <code>ShellClientSurface</code> provides
...
public class ShellClientSurface extends AbstractShellSurface {
	 ...
	 *            The {@link ShellDisplay} where this <code>ShellClientSurface</code>
	 *            lives on.
	 * @param clientWindow
	...
	public void handleMapRequest(final ShowRequestEvent event) {
		if ((getDisplayRenderArea() == null)
			...
			setDisplayRenderArea((DisplayRenderArea) event.getEventSource());
		}
		requestShow();
	}
