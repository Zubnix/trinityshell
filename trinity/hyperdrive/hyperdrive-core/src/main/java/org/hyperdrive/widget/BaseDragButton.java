...
import org.hyperdrive.api.widget.DragButton;
 ...
 * @since 1.0
 */
@HasView(DragButton.View.class)
public class BaseDragButton extends BaseButton implements DragButton {
	 ...
	 * 
	 */
	public BaseDragButton() {
		stopDrag();
	}

	@Override
	public DragButton.View getView() {
		return (DragButton.View) super.getView();
	}
	...
	private void doDragCycles(final BaseManagedMouse mousePointer) {
		while (isStarted()) {
			mousePointer.refreshPositionInfo();
	...
	public void startDrag(final MouseInput input) {
		if (!isStarted()) {
			final BaseManagedMouse mousePointer = getManagedDisplay()
					.getManagedMouse();
