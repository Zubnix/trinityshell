...
import org.trinity.display.x11.impl.AtomImpl;
 ...
 * @param <T>
 */
public abstract class AbstractXProperty<T> extends AtomImpl implements
	 ...
	 */
	protected FlexDataContainer getRawPropertyValue(final XWindowImpl window) {
		...
		final FlexDataContainer rawPropertyValueContainer = new FlexDataContainer(ByteBuffer
				.wrap(rawContents));

		return rawPropertyValueContainer;
	}

	protected XPropertyInstanceInfo readXAtomMetaInfo(final FlexDataContainer rawPropertyValue) {
	...
	public PropertyInstance<T> getPropertyInstance(final PlatformRenderArea platformRenderArea) {
		final FlexDataContainer rawDataContainer = getRawPropertyValue((XWindowImpl) platformRenderArea);
