...
public class CloseRequestProtocolHandler implements XDisplayProtocolHandler {
	...
	public Map<String, Object> handleDipslayProtocol(	final XWindow xWindow,
		...
		final Map<String, Object> wmProtocolsValue = this.xPropertyCache
				.getXProperty(xWindow, "WM_PROTOCOLS");
		...
		if (protocols.contains("WM_DELETE_WINDOW")) {
			...
			final int wmDeleteWindowId = this.xAtomCache
			...
			LibXcb.xcb_send_event(	this.xConnection.getConnectionReference(),
									...
									xcb_event_mask_t.XCB_EVENT_MASK_NO_EVENT
											.swigValue(),
									new xcb_generic_event_t(xcb_client_message_event_t
																	.getCPtr(client_message_event_t),
															true));
		} else {
