...
/**
 * An <code>XNativeCall</code> wraps and and handles a JNI call. It
 * splits the original operation of calling a basic jni method into:
 * implementation through <code>nativeCallImpl()</code> calling through
 * <code>exec</code> and result fetching through <code>getResult()</code>.
 * <p>
 ...
 *            The arguments type.
 */
public abstract class XNativeCall<R, D, A> {
	private D displayPeer;
	private A[] args;
	...
	public boolean exec() {
		...
		if (nativeBufferHelper.isWriteEnabled()) {
			...
			final boolean error = nativeCallImpl();
			nativeBufferHelper.getBuffer().compact();
			nativeBufferHelper.getBuffer().flip();
			return error;
		} else {
			throw new Error(
					"Program bug! Can not execute native call. Result buffer is not yet marked as done reading.");
		}
	}
	 ...
	 * @return
	 */
	protected abstract boolean nativeCallImpl();
	 ...
	 *            A display peer.
	 */
	void setDisplayPeer(final D displayPeer) {
