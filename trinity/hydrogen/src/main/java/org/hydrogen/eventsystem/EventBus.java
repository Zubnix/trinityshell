 ...
 */
package org.hydrogen.eventsystem;
...
// rawtypes because javac is 'teh suck' when it comes to generics (unlike
// eclipse compiler).
public class EventBus implements EventHandlerManager, EventManager {
	 ...
	 */
	public EventBus() {
		this.eventHandlerSwitch = Collections
				.synchronizedMap(new HashMap<Type, List<EventHandler>>());
	}
	...
	public <T extends Type> void addEventHandler(
			final EventHandler<? extends Event<T>> eventHandler, final T type,
			final int index) {
		synchronized (getEventHandlerSwitch()) {
			final List<EventHandler> eventHandlersList = Collections
					.synchronizedList(new ArrayList<EventHandler>());

			if (getEventHandlerSwitch().containsKey(type)) {
				eventHandlersList.addAll(getEventHandlerSwitch().get(type));
			}

			getEventHandlerSwitch().put(type, eventHandlersList);
	...
	public void fireEvent(final Event<? extends Type> event) {
		synchronized (getEventHandlerSwitch()) {
			if (this.eventHandlerSwitch.containsKey(event.getType())) {
				...
				final List<EventHandler> listToIterate = new ArrayList<EventHandler>(
						eventHandlersList.size());
				listToIterate.addAll(eventHandlersList);

				synchronized (eventHandlersList) {
					if (!eventHandlersList.isEmpty()) {
						for (final EventHandler<Event<Type>> eventHandler : listToIterate) {
							eventHandler.handleEvent((Event<Type>) event);
						}
					}
				}
			}
	...
	public <T extends Type> void removeEventHandler(
			final EventHandler<? extends Event<T>> eventHandler, final T type) {
		synchronized (getEventHandlerSwitch()) {
			final List<EventHandler> eventHandlersList = getEventHandlerSwitch()
					.get(type);
			synchronized (eventHandlersList) {
				eventHandlersList.remove(eventHandler);
			}
		}
	}
