...
public class BaseClientMessageEvent extends BaseDisplayEvent implements
	 ...
	 * @param data
	 */
	public BaseClientMessageEvent(
			final DisplayEventSource platformRenderAreaArgument,
			final Atom dataType, final int dataFormat, final byte[] data) {
		super(ClientMessageEvent.TYPE, platformRenderAreaArgument);
	...
	public String toString() {
		...
		return String
				.format("%s\tDetails: message type: %s - data format: %d - raw data: %s",
						super.toString(), getMessageType().getAtomName(),
						getDataFormat(), getData());
	}
}
