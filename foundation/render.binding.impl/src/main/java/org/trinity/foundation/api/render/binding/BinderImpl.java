...
public class BinderImpl implements Binder {
	...
	private static final Cache<Class<?>, Cache<String, Optional<Method>>> getterCache = CacheBuilder
			.newBuilder().build();
	private static final Cache<Class<?>, Field[]> childViewCache = CacheBuilder
			.newBuilder().build();
	...
	@Override
	public void bind(final Object model, final Object view) {
		checkNotNull(model);
		checkNotNull(view);

		bindViewElement(model, view, Optional.<DataContext> absent(),
	...
	@Override
	public void updateBinding(final Object model, final String propertyName) {
		checkNotNull(model);
		checkNotNull(propertyName);

		updateDataContextBinding(model, propertyName);
	...
	protected void updateDataContextBinding(final Object model,
		...
		for (final Entry<Object, DataContext> dataContextByViewEntry : dataContextByView
				.entrySet()) {

			final Object view = dataContextByViewEntry.getKey();
			final DataContext dataContext = dataContextByViewEntry.getValue();
			final Optional<DataContext> optionalDataContext = Optional
					.of(dataContext);
			...
			if (dataContext.value().startsWith(propertyName)) {
				...
				bindViewElement(model, view, optionalDataContext,
						optionalInputSignals, optionalObservableCollection,
	...
	protected void updateProperties(final Object model,
		...
		try {
			...
			for (final Object view : views) {
				final PropertySlots propertySlots = this.propertySlotsByView
						.get(view);
				if (propertySlots == null) {
					continue;
				}
				for (final PropertySlot propertySlot : propertySlots.value()) {
					final String propertySlotPropertyName = propertySlot
							.propertyName();
	...
	protected void bindViewElement(
		...
		final Optional<DataContext> optionalDataContext = optionalFieldLevelDataContext
				.or(Optional.<DataContext> fromNullable(viewClass
						.getAnnotation(DataContext.class)));
		final Optional<InputSignals> optionalInputSignals = optionalFieldLEvelInputSignals
				.or(Optional.<InputSignals> fromNullable(viewClass
		...
		final Optional<ObservableCollection> optionalObservableCollection = optionalFieldLevelObservableCollection
				.or(Optional.<ObservableCollection> fromNullable(viewClass
		...
		final Optional<PropertySlots> optionalPropertySlots = optionalFieldLevelPropertySlots
				.or(Optional.<PropertySlots> fromNullable(viewClass
						.getAnnotation(PropertySlots.class)));

		Object dataContext = inheritedDataContext;
		if (optionalDataContext.isPresent()) {
			final Optional<Object> optionalDataContextValue = getDataContextValueForView(
					dataContext, view, optionalDataContext.get());
		...
		if (optionalInputSignals.isPresent()) {
			final InputSignal[] inputSignals = optionalInputSignals.get()
					.value();
			bindInputSignals(dataContext, view, inputSignals);
		}

		if (optionalObservableCollection.isPresent()) {
			final ObservableCollection observableCollection = optionalObservableCollection
					.get();
			bindObservableCollection(dataContext, view, observableCollection);
		}
		...
		bindChildViewElements(dataContext, view);
	}
	...
	protected void bindObservableCollection(final Object dataContext,
		...
		try {
			...
			final Optional<Method> collectionGetter = findGetter(
					dataContext.getClass(), collectionProperty);
			if (!collectionGetter.isPresent()) {
				return;
			}

			final Object collection = collectionGetter.get()
					.invoke(dataContext);

			checkArgument(
					collection instanceof EventList,
					format("Observable collection must be bound to a property of type %s @ dataContext: %s, view: %s, observable collection: %s",
							EventList.class.getName(), dataContext, view,
	...
	protected void handleListChanged(final Object view,
		...
		while (listChanges.next()) {
			...
			case ListEvent.DELETE: {
				final Object removedObject = shadowChildDataContextList
						.remove(sourceIndex);
				checkNotNull(removedObject);

				final Set<Object> removedChildViews = BinderImpl.this.viewsByDataContextValue
						.get(removedObject);
				for (final Object removedChildView : removedChildViews) {
					BinderImpl.this.childViewDelegate.destroyView(view,
							removedChildView, sourceIndex);
				}
			...
			case ListEvent.INSERT: {
				...
				final Object childView = BinderImpl.this.childViewDelegate
						.newView(view, childViewClass, sourceIndex);
				checkNotNull(childView);
				bind(childViewDataContext, childView);
			...
			case ListEvent.UPDATE: {
				if (listChanges.isReordering()) {
					...
					for (int i = 0; i < reorderings.length; i++) {
						...
						final Object childViewDataContext = changeList
								.get(sourceIndex);
						...
						for (final Object changedChildView : changedChildViews) {
							BinderImpl.this.childViewDelegate
									.updateChildViewPosition(view,
											changedChildView, i, newPosition);
						}
					}
				} else {
					final Object newChildViewDataContext = changeList
							.get(sourceIndex);
					final Object oldChildViewDataContext = shadowChildDataContextList
							.set(sourceIndex, newChildViewDataContext);
					checkNotNull(oldChildViewDataContext);
					checkNotNull(newChildViewDataContext);

					final Object childView = BinderImpl.this.viewsByDataContextValue
							.get(oldChildViewDataContext);
					checkNotNull(childView);

					bind(newChildViewDataContext, childView);
				}
	...
	protected void bindInputSignals(final Object dataContext,
			final Object view, final InputSignal[] inputSignals) {
		checkNotNull(dataContext);
		checkNotNull(view);
		...
		for (final InputSignal inputSignal : inputSignals) {
			...
			this.inputListenerInstallerDelegate.installInputListener(inputType,
					view, dataContext, inputSlotName);
		}
	}
	...
	protected void registerBinding(final Object dataContext, final Object view) {
		...
		if (oldDataContext != null) {
			final Set<Object> oldDataContextViews = this.viewsByDataContextValue
					.get(oldDataContext);
			if (oldDataContextViews != null) {
				oldDataContextViews.remove(view);
			}
		}
		Set<Object> dataContextViews = this.viewsByDataContextValue
				.get(dataContext);
		if (dataContextViews == null) {
			dataContextViews = Sets
	...
	protected void bindPropertySlots(final Object dataContext,
			final Object view, final PropertySlots propertySlots) {
		checkNotNull(dataContext);
		checkNotNull(view);
	...
	protected void bindPropertySlot(final Object dataContext,
			final Object view, final PropertySlot propertySlot) {
		checkNotNull(dataContext);
		checkNotNull(view);
		...
		try {
			...
			if (propertySlotDataContext.isEmpty()) {
				...
				Optional<Object> optionalRelativeDataContext = getDataContextValue(
						dataContext, propertySlotDataContext);
			...
			final Optional<Method> getter = findGetter(
					propertyDataContext.getClass(), propertyName);
			if (getter.isPresent()) {
				final Object propertyInstance = getter.get().invoke(
						propertyDataContext);
	...
	protected void invokePropertySlot(final Object view,
			final PropertySlot propertySlot, final Object propertyValue) {
		checkNotNull(view);
		checkNotNull(propertySlot);
		...
		try {
			...
			final Class<?>[] viewMethodArgumentTypes = propertySlot
					.argumentTypes();
			final Method targetViewMethod = view.getClass().getMethod(
					viewMethodName, viewMethodArgumentTypes);
			final Class<? extends PropertyAdapter<?>> propertyAdapterType = propertySlot
					.adapter();
			@SuppressWarnings("rawtypes")
			final PropertyAdapter propertyAdapter = propertyAdapterType
					.newInstance();
	...
	protected Optional<Object> getDataContextValueForView(
			final Object parentDataContextValue, final Object view,
		...
		if (dataContextByView == null) {
			...
			this.dataContextByViewByParentDataContextValue.put(
					parentDataContextValue, dataContextByView);
		}
	...
	protected Field[] findChildViews(final Class<?> modelClass)
			throws ExecutionException {
		return childViewCache.get(modelClass, new Callable<Field[]>() {
			@Override
			public Field[] call() throws Exception {
				final List<Field> possibleChildViews = new ArrayList<Field>();
				final Field[] childViewElements = modelClass
						.getDeclaredFields();
				for (final Field childViewElement : childViewElements) {
					...
					for (final Class<?> validViewElementType : BinderImpl.this.viewElementTypes
						...
						interestedViewElement |= validViewElementType
								.isAssignableFrom(childViewElement
										.getDeclaringClass());
	...
	protected void bindChildViewElements(final Object inheritedModel,
		...
		try {
			...
			for (final Field childViewElement : childViewElements) {
				...
				bindViewElement(inheritedModel, childView,
						optionalFieldDataContext, optionalFieldInputSignals,
	...
	protected Optional<Object> getDataContextValue(final Object model,
		...
		try {
			...
			for (final String propertyName : propertyNames) {
				...
				final Optional<Method> foundMethod = findGetter(
						currentModelClass, propertyName);
			...
			throw new BindingError(
					String.format(
							"Can not access getter on %s. Is it a no argument public method?",
							currentModel), e);
		} catch (final IllegalArgumentException e) {
			// TODO explanation
			throw new BindingError("", e);
			...
			public Optional<Method> call() {
				...
				try {
					...
					try {
						...
						throw new BindingError(
								format("Property %s is not accessible on %s. Did you declare it as public?",
										propertyName, modelClass.getName()), e2);
					}
				} catch (final SecurityException e3) {
					throw new BindingError(
							format("Property %s is not accessible on %s. Did you declare it as public?",
									propertyName, modelClass.getName()), e3);
				}
