...
public class TestCirculateNotifyHandler {
    ...
    private int targetWindowId = 123;
    ...
    public void testEventHandling() {
        ...
        Optional<StackingChangedNotify> stackingChangedNotifyOptional = circulateNotifyHandler
    ...
    public void testGetTarget() {
        ...
        when(xcb_circulate_notify_event_t_window_get(anyLong(),
                                                     any())).thenReturn(targetWindowId);

        DisplaySurface displaySurface = mock(DisplaySurface.class);
        when(displaySurface.getDisplaySurfaceHandle()).thenReturn(new XWindowHandle(targetWindowId));
        when(xWindowPool.getDisplaySurface(any())).thenAnswer(invocation -> {
            Object arg0 = invocation.getArguments()[0];
            XWindowHandle xWindowHandle = (XWindowHandle) arg0;
        ...
        Optional<DisplaySurface> target = circulateNotifyHandler.getTarget(xcb_generic_event);


        //then
        //the correct DisplaySurface is returned
        ArgumentCaptor<XWindowHandle> windowHandleArgumentCaptor = ArgumentCaptor.forClass(XWindowHandle.class);
