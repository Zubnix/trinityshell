...
public class WmState extends AbstractCachedProtocol<int[]> {
	...
	WmState(@Named("WindowManager") final ListeningExecutorService wmExecutor,
	...
	protected ListenableFuture<Optional<int[]>> queryProtocol(final DisplaySurface xWindow) {
		...
		final xcb_get_property_cookie_t get_wm_state_cookie = xcb_get_property(	this.xConnection.getConnectionRef(),
																				(short) 0,
																				winId.intValue(),
																				getProtocolAtomId(),
																				getProtocolAtomId(),
		...
		return this.wmExecutor.submit(new Callable<Optional<int[]>>() {
			...
			public Optional<int[]> call() {
				...
				final xcb_get_property_reply_t get_wm_state_reply = xcb_get_property_reply(	WmState.this.xConnection.getConnectionRef(),
																							get_wm_state_cookie,
																							e);
				...
				final ByteBuffer wm_state_property_value = xcb_get_property_value(get_wm_state_reply).order(ByteOrder.nativeOrder());
